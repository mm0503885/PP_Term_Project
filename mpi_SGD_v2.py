...
def train(input_train, target_train, hidden_nodes, epochs, batch_size, lr):
    ...
    for e in range(epochs):
        ...
        for imgs, labels in zip(imgs_batch, labels_batch):
            ...
            delta_w1_buf = None
            if comm.rank == 0:
                delta_w0_buf = np.empty([comm.size, input_nodes, hidden_nodes], dtype = 'float64')
                delta_w1_buf = np.empty([comm.size, hidden_nodes, output_nodes], dtype = 'float64')
            comm.Barrier()
            ...
            comm.Barrier()
            if comm.rank == 0:
                delta_w0 = functools.reduce(np.add, delta_w0_buf) / comm.size
                delta_w1 = functools.reduce(np.add, delta_w1_buf) / comm.size
            comm.Barrier()
            comm.Barrier()
